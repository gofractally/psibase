...
Ensure the local `psibase-package` extension is installed (it is not on the Marketplace). It lives in `.vscode/psibase-package/` and provides diagnostics, hover help, inlay hints, rename, and correlated-name navigation for psibase package naming.
...
Then install it with **Developer: Install Extension from Location...** and select `.vscode/psibase-package`.
