...
void SymbolSys::buySymbol(SID symbol)
{
   ...
   at<TokenSys>().debit(TokenSys::sysToken, buyer, salePrice, buyerMemo);
   at<TokenSys>().credit(TokenSys::sysToken, seller, salePrice, sellerMemo);
...
SymbolLengthRecord SymbolSys::getSymbolType(size_t numChars)
{
   ...
   auto symbolType = db.open<SymbolLengthTable_t>().get_index<0>().get(key);
...
void SymbolSys::updatePrices()
{
   ...
   auto priceAdjustmentIdx = db.open<PriceAdjustmentSingleton_t>().get_index<0>();
