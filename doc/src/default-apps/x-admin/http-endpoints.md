...
The HTTP API allows access to `x-admin` for scripts and UIs. Most APIs use JSON (`Content-Type` should be `application/json`).

| Method | URL                          | Description                                                                             |
|--------|------------------------------|-----------------------------------------------------------------------------------------|
| `GET`  | `/services/*`                | Returns the wasm for a local service                                                    |
| `PUT`  | `/services/*`                | Sets the wasm for a local service                                                       |
| `GET`  | `/admin_accounts`            | Returns a JSON list of all on-chain accounts that are authorized to administer the node |
| `POST` | `/admin_accounts`            | Adds or removes the account from the set of administrator accounts for the node.        |
...
| `POST` | `/native/admin/push_boot`    | Begins booting the chain                                                                |
...
The [x-peers](../../default-apps/x-peers.md#http-endpoints) service also provides some endpoints for node admins.

## Local services

`/services/<name>` allows `GET`/`PUT` access to local service wasms. The `Content-Type` should be `application/wasm`.

## Managing access

`GET` `/admin_accounts` returns a JSON list of all on-chain accounts that are authorized to administer the node.

`POST` to `/admin_accounts` adds or removes an account from the list of administrators.

| Field     | Type    | Description                                    |
|-----------|---------|------------------------------------------------|
| `account` | String  | An account name                                |
| `admin`   | Boolean | Whether the account should be an administrator |
...
| `peers`              | Array   | A list of peer URLs that the server may connect to. To manage the active connections, see [peer management](../x-peers.md)                                                           |
...
| Field  | Type   | Description                                                                                                                                                                  |
|--------|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| filter | String | The [filter](../../run-infrastructure/administration/logging.md#log-filters) for this websocket. If no filter is provided, the default is to send all possible log messages. |
| format | String | The [format](../../run-infrastructure/administration/logging.md#log-formatters) for log messages. If no format is provided, the default is JSON.                             |
