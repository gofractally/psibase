...
namespace arbtrie
{
   ...
   /**
       ...
       */
   inline int binary_node::alloc_size(clone_config cfg)
   {
      auto desired = round_up_multiple<64>(
          round_up_multiple<64>(sizeof(binary_node) + 4*round_up_multiple<32>(cfg.spare_branches)) +
   ...
   inline int binary_node::alloc_size(const binary_node* src, clone_config cfg)
   {
      auto desired = round_up_multiple<64>(
                                      ...
                                      is_value_type auto val)
   {
      auto desired = round_up_multiple<64>(
          round_up_multiple<64>(sizeof(binary_node) +
                                4*round_up_multiple<32>(cfg.spare_branches + src->num_branches())) +
          round_up_multiple<16>(src->key_val_section_size() + 4+ calc_key_val_pair_size(key, val) +
                                cfg.spare_space));
   ...
   inline binary_node::binary_node(int_fast16_t asize, object_id nid, clone_config cfg)
  ...
  //    TRIEDENT_WARN("MAKE BINARY  size:", asize, " id: ", nid );
   ...
   {
      assert(not cfg.update_prefix);
      assert(alloc_size(src, cfg, key, val) <= asize);
  ...
  //    TRIEDENT_WARN("CLONE ADD!!! src kv section size ", src->key_val_section_size(),
   //                 " cfg.spareb: ", cfg.spare_branches, " nbranch: ", src->num_branches(),
    //                " nbc: ", int(_branch_cap) );
      ...
      key_index kidx;
      kidx.pos = _alloc_pos;
      if constexpr (std::same_as<object_id, decltype(val)>)
      {
         kidx.type       = key_index::obj_id;
         kvp->_val_size  = sizeof(object_id);
         kvp->value_id() = val;
      }
      else if constexpr (std::same_as<value_view, decltype(val)>)
      {
         assert(key.size() < max_inline_value_size);
         kvp->_val_size = val.size();
         memcpy(kvp->val_ptr(), val.data(), val.size());
      }
      // locate where to insert the new key
      auto kh = key_hash(key);
      auto vh = value_hash(val);

      const auto lb     = src->lower_bound_idx(key);
      const auto lb1    = lb + 1;
      ...
      for( int i = 0; i < src->num_branches(); ++i ) {
         ...
         auto nkvp = get_key_val_ptr(i);
         TRIEDENT_DEBUG( i,"]  src: ", kvp->key(), " dst: ", nkvp->key(),
                               "   srckh: ", int(src->key_hashes()[i]), "  dstkh: ", int(key_hashes()[i]));
   ...
   inline void binary_node::insert(key_view key, is_value_type auto val)
   {
      assert(get_type() == node_type::binary);
    ...
    //     _branch_cap= num_branches() + 2;
      ...
      const auto vh = value_hash(val);

      const auto lb     = lower_bound_idx(key);
      const auto lb1    = lb + 1;
