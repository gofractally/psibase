...
//            return get_view_type<typename T::value_type>();
        if constexpr( reflect<T>::is_struct )  {
            using view_type = typename reflect<T>::template safe_proxy<safe::flat_view_proxy_impl>;
            return (view_type*)(nullptr);
        }
        else if constexpr( std::is_trivially_copyable<T>::value ) 
            return (T*)(nullptr);
        else
            return (safe::flat<T>*)nullptr; 
    }
        ...
        public:
            auto& operator[]( uint32_t index ) {
                ...
                if constexpr( std::is_same<T,std::string>::value ) {
                    ...
                    return *reinterpret_cast<safe::flat<std::string>*>( ptr_array[index].get< std::vector<T> >() );
                } else if constexpr( contains_offset_ptr<T>() ) {
                    auto ptr_array =  reinterpret_cast<offset_ptr*>(_data);
                    return *reinterpret_cast<safe::flat_view<T>*>( ptr_array[index].get< std::vector<T> >() );
                } else if constexpr ( reflect<T>::is_struct ) { /// the data is a series of packed T
                    const auto offset = index * flatpack_size<T>(); 
                    return *reinterpret_cast<safe::flat_view<T>*>( &_data[offset] );
                } else if constexpr ( std::is_trivially_copyable<T>::value ) {
    ...
    auto offset_ptr::get()const {
        ...
        if constexpr( is_flat_ptr<T>::value ) {
            return reinterpret_cast< decltype(safe::get_view_type<typename T::value_type>()) >(ptr+4);
        } else if constexpr( reflect<T>::is_struct ) {
            return reinterpret_cast<safe::flat_view<T>*>(ptr);
        } else if constexpr( std::is_same_v<std::string,T> ) {
            return reinterpret_cast< safe::flat<std::string>* >(ptr);
        } else if constexpr( is_std_vector<T>::value ) {
            return reinterpret_cast< safe::flat<T>* >(ptr);
        } else {
