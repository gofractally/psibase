...
function(psibase_package)
    ...
    foreach(service IN LISTS _SERVICES)
        if (NOT "${service}" MATCHES "^[-0-9a-z][-0-9a-z]?[-0-9a-z]?[-0-9a-z]?[-0-9a-z]?[-0-9a-z]?[-0-9a-z]?[-0-9a-z]?[-0-9a-z]?[-0-9a-z]?$")
            message(FATAL_ERROR "Invalid account name ${service}")
        endif()
        if ("${service}" MATCHES "^-.*|.*-$|.*--.*")
            message(FATAL_ERROR "Invalid account name ${service}")
        endif()
