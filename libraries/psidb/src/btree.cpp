...
struct transaction {
   ...
   // when the barrier was triggered.
   ...
   std::atomic<barrier_state> barrier;
   // A thread may only read page links while holding the lock.
   // Either set_barrier happens before lock OR lock happens before set_barrier
   //
   // If set_barrier happens before lock, then the thread that locks must
   // see all changes to memory made before set_barrier.
   //
   // If lock happens before set_barrier, then unlock must synchronize
   // with the receipt of the notification.
   //
   // Returns true if no waiting is required
   //
   // Requires that the the previous barrier has completed.
   bool set_barrier() {
      auto state = barrier.load(std::memory_order_relaxed);
      ...
      while(true) {
         ...
         ++new_state.version;
         if(barrier.compare_exchange_weak(state, new_state, std::memory_order_release, std::memory_order_relaxed)) {
            return new_state.remaining == 0;
         }
      }
   }
   unsigned lock() {
      ...
      while(true) {
         ...
         if(barrier.compare_exchange_weak(state, new_state, std::memory_order_relaxed, std::memory_order_acquire)) {
            return saved;
         }
      }
   }
   void unlock(unsigned prev_version) {
      ...
      while(true) {
         ...
         if(new_state.version != prev_version) {
            --new_state.remaining;
            // In this case, we need a release barrier
         }
         if(barrier.compare_exchange_weak(state, new_state, std::memory_order_relaxed, std::memory_order_acquire)) {
            if(new_state.version != prev_version && new_state.remaining == 0) {
               // At this point, every thread that is still
               // in the critical section definitely has the new version.
               // Notify the caller
            }
            return;
         }
