...
TEST_CASE("flat schema", "[flat schema]")
{
   ...
   auto convert_to_js_type = [&](const std::string& str, bool& is_repeated,
                                 bool& is_prim) -> std::string
   {
      ...
      if (decay == "char")
         ...
         return "Int16LE";
      else if (decay == "uint16")
         return "UInt16LE";
      else if (decay == "int32")
         return "Int32LE";
      else if (decay == "uint32")
         return "UInt32LE";
      else if (decay == "int64")
         return "Int64LE";
      else if (decay == "uint64")
         return "UInt64LE";
      else if (decay == "bool")
         return "UInt8";
          ...
          [&](auto i)
          {
             if constexpr (std::is_same_v<clio::object_type, decltype(i)>)
             {
                ...
                for (const auto& m : i.members)
                {
                   ...
                   {
                      std::cout << "    get " << m.name << "() { \n";
                      std::cout << "        offsetptr = bops.readUInt16LE( this.buffer, "
                                << m.offset << " )\n";
                   ...
                   else if (name == "string")
                   {
                      ...
                      std::cout
                          << "        strbuf = bops.subarray( this.buffer, offsetptr+4, size );\n";
                      std::cout << "        return bops.to( strbuf, encoding=\"utf8\" );\n";
                      std::cout << "    }\n";
                   }
                   else if (is_prim)
                   {
                      std::cout << "    get " << m.name << "() { return bops.read" << name
                                << "( this.buffer, " << m.offset << " ) }\n";
                   }
                   else
                   {
