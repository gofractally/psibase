...
namespace trie
{
   struct arena
   {
      ...
      struct deleater
      {
         ...
         void operator()(pointer& o)const
         {
            auto obj = o.get();
            if( obj ) {
               arena::destruct(obj);
               arena::get_arena(obj).free(obj);
            }
         }
      };

      template <typename T>
      using unique_ptr = std::unique_ptr<T, deleater<T>>;
      ...
      template <typename T, typename... Ts>
      auto construct_unique(Ts&&... args)
      {
         return unique_ptr<T>(new (alloc(sizeof(T))) T(std::forward<Ts>(args)...));
      }

      template <typename T>
      static void destruct(offset_ptr<T>& obj)
      {
         destruct(obj.get());
         obj.reset();
      }
      ...
      static arena& get_from(const void* p) { return *get_page(p)->alloc_arena; }

      static void free(void* p) { get_page(p)->alloc_arena->deallocate(p); }

      static arena& get_arena(void* p) { return *get_page(p)->alloc_arena; }
