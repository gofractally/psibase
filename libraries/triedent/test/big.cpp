      ...
      try
      {
         ...
         while (not done.load(std::memory_order_acquire))
         {
            ...
            while (r.load(std::memory_order_relaxed) == v)
            {
               ...
               bool     found = rs->get_less_than(rr, std::string_view((char*)&h, sizeof(h)),
                                                  &found_key, &found_value, &result_roots);
               if (found)
               {
   ...
   for (uint64_t i = 0; i < insert_count; ++i)
   {
      try
      {
         ...
         if (i % status_count == 0)
         {
            ...
            read_start      = read_end;
            uint64_t r_per_sec =
                delta_read / ((std::chrono::duration<double, std::milli>(delta).count() / 1000));
