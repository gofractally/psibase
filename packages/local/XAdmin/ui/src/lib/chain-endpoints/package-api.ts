import type { ManifestPayload, PackageMeta } from "./package-extraction";
...
export async function callPreinstall(meta: PackageMeta): Promise<void> {
    const response = await fetch(
        `${siblingUrl(null, "x-packages", "/preinstall")}`,
        {
            method: "POST",
            headers: {
    ...
    if (!response.ok) {
        throw new Error(
            `Failed to install package: ${response.status} ${response.statusText}`,
        );
    }
}
...
export async function installManifest(
    ...
    const response = await fetch(
        `${siblingUrl(null, "x-packages", "/manifest")}/${sha256.toUpperCase()}`,
        {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
...
export async function callPostinstall(meta: PackageMeta): Promise<void> {
    const response = await fetch(
        `${siblingUrl(null, "x-packages", "/postinstall")}`,
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
