...
mod service {
    ...
    use crate::state::{
        ...
        ensure_sender_is_member, group_members, members_of,
        next_server_msg_id, open_conversation,
        sessions_for_user, socket_session, targeted_message_was_delivered,
        upsert_socket_session, upsert_targeted_say_pending, complete_targeted_say_ack,
                    ...
                    Ok(()) => {
                        ...
                        let recipient_sockets: Result<Vec<i32>, StateError> = if let Some(target) = to {
                            if !crate::state::is_conversation_member(&conversation_id, target) {
                                ...
                                upsert_targeted_say_pending(crate::state::tables::TargetedSayPendingRow {
                                    conversation_id: conversation_id.clone(),
                                ...
                                if targeted_message_was_delivered(&conversation_id, &client_msg_id, target) {
                                    ...
                                    sockets.extend(sessions_for_user(target));
                                }
                                Ok(sockets)
                            }
                        } else {
                        ...
                        recipient_sockets.map(|sockets| (sockets, frame))
                    }
                };
            out
        }
    ...
    fn subjective_prepare_targeted_ack(
        conversation_id: String,
        acker: AccountNumber,
        client_msg_id: String,
    ) -> Result<(Vec<i32>, ServerFrame), StateError> {
        ::psibase::subjective_tx! {
            let out: Result<(Vec<i32>, ServerFrame), StateError> =
                match complete_targeted_say_ack(&conversation_id, &client_msg_id, acker) {
                    Err(err) => Err(err),
                    Ok(row) => {
                        let sockets = sessions_for_user(row.sender);
                        Ok((
                            sockets,
                            ServerFrame::Message {
                                conversation_id: row.conversation_id.clone(),
                                from: row.sender.into(),
                                body: row.body.clone(),
                                server_msg_id: row.server_msg_id,
                                server_time: row.server_time,
                                client_msg_id: Some(row.client_msg_id.clone()),
                                client_time: row.client_time,
                                to: Some(row.target.into()),
                            },
                        ))
                    }
                };
    ...
    fn handle_ack(
        ...
        let conv_for_err = conversation_id.clone();
        let result = if let Some(client_msg_id) = client_msg_id {
            subjective_prepare_targeted_ack(conversation_id, sender, client_msg_id)
        } else {
            subjective_prepare_ack(conversation_id, sender, server_msg_id)
        };
        match result {
            Err(err) => send_state_error(socket, err, Some(conv_for_err)),
            Ok((recipient_sockets, frame)) => {
