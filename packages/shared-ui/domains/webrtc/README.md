# WebRTC session (`packages/shared-ui/domains/webrtc`)

Reusable client infrastructure for psibase apps that need **x-wrtcsig**
websocket transport, peer presence, and WebRTC signaling.

**New client?** Start with [`webrtc-client-doc.md`](./webrtc-client-doc.md) —
APIs and the Homepage presence-mount reference files for this PR tip.

Mount `WebRtcSessionProvider` at your **app root for one page load**. It owns
the websocket lifecycle for that document only — it does **not** survive
subdomain navigation, tab close, or cross-origin hops.

## Usage

```tsx
import {
    WebRtcSessionProvider,
    useWebRtcSession,
} from "@shared/domains/webrtc";

function MyAppRoot() {
    return (
        <WebRtcSessionProvider authTokenProvider={getMyAppQueryToken}>
            <MyFeature />
        </WebRtcSessionProvider>
    );
}

function MyFeature() {
    const { client, signaling, connectionState, registerHandlers } =
        useWebRtcSession();

    useEffect(() => {
        return registerHandlers({
            presenceSnapshot: (frame) => {
                /* seed from frame.peers */
            },
        });
    }, [registerHandlers]);

    return <div>{connectionState}</div>;
}
```

## Exports

| Module | Purpose |
| --- | --- |
| `components/webrtc-session-provider` | React provider + `useWebRtcSession()` |
| `lib/realtime-client` | Websocket client for `/ws` |
| `lib/webrtc-signaling-client` | joinSession / signal / leaveSession helpers |
| `lib/realtime-protocol` | Zod schemas + frame types |
| `lib/realtime-schemas` | ICE + presence schemas |

## Debug

- `localStorage.setItem('webrtc-realtime-trace', '1')` — verbose websocket transport logs
- Pass `debugLog` to the provider for app-specific frame tracing
