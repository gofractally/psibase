...
export { mergeRealtimeHandlers };

/**
 * Websocket client for `x-wrtcsig` `/ws` with subprotocol `psibase.realtime.v1`.
 * (Some docs say `x-webrtc-sig`; that exceeds the psibase account name length.)
 */
export class RealtimeClient {
    ...
    constructor(opts: RealtimeClientOptions = {}) {
        ...
        this.health = new HealthWatchdog({
            ...
            isActive: () => !this.userClosed && this.welcomeReady,
    ...
    get isWelcomeReady(): boolean {
        return this.welcomeReady;
    }
    ...
    private rebuildHandlersFromLayers(): void {
        this.handlers = mergeRealtimeHandlers(
            ...this.handlerLayers.values(),
        );
    }
    ...
    private noteWelcomeClosed(
        wasWelcomeReady: boolean,
        connectStartedAt: number,
    ) {
        if (!wasWelcomeReady) return;
        const ageMs =
            this.welcomeAt > 0
    ...
    private sendJson(payload: Record<string, unknown>): void {
        if (!this.welcomeReady || this.ws?.readyState !== WebSocket.OPEN) {
        ...
        await runRealtimeConnect(
            {
                ...
                get welcomeReady() {
                    return self.welcomeReady;
                },
                set welcomeReady(v: boolean) {
                    self.welcomeReady = v;
                },
                get ws() {
                    return self.ws;
                ...
                noteWelcomeClosed: (ready, at) =>
                    self.noteWelcomeClosed(ready, at),
            },
            force,
            () => self.isStableReady(),
