    ...
    try {
        ...
        ws.onclose = (ev) => {
            ...
            realtimeTraceLog("ws.onclose", {
                ...
                welcomeReady: host.welcomeReady,
                code: ev?.code,
                reason: ev?.reason,
            ...
            const wasWelcomeReady = host.welcomeReady;
            host.debugLog?.("ws close", {
                instanceId: host.clientInstanceId,
                welcomeReady: wasWelcomeReady,
                elapsedMs: Date.now() - connectStartedAt,
                code: ev?.code,
                reason: ev?.reason,
                wasClean: ev?.wasClean,
            ...
            host.noteWelcomeClosed(wasWelcomeReady, connectStartedAt);
            host.setLastError(
