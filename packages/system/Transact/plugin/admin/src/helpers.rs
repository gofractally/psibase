...
pub fn get_proofs(
    ...
    for group in action_claim_groups {
        let plugin_ref = PluginRef::new(&group.claimant, "plugin", "transact-hook-action-auth");
        proofs.extend(on_action_auth_proofs(
            plugin_ref,
            &group.claims,
            tx_hash,
        )?);
...
pub fn make_transaction(
...
) -> (Transaction, Vec<ActionClaims>) {
    let action_claim_groups = ActionsLedger::take_action_claims();
    ...
    for group in &action_claim_groups {
        claims.extend(group.claims.iter().cloned());
    }
