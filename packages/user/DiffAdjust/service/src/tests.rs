...
fn test_max_ppm_unclamped(mut chain: psibase::Chain) -> Result<(), psibase::Error> {
    ...
    // Max decrease ppm on a separate instance.
    let nft_id = Wrapper::push_from(&chain, alice)
    ...
    let decrease_factor = 1.0 - f64::from(max_ppm) / ONE_MILLION;
    let expected_decrease = (10_000.0 * decrease_factor).max(1.0) as u64;
    assert_eq!(
        Wrapper::push(&chain).get_diff(nft_id).get()?,
        expected_decrease
    );

    // set_ppm also accepts values above the old 100% cap.
    let nft_id = Wrapper::push_from(&chain, alice)
