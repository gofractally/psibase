...
pub mod service {
    ...
    /// * `finger_print` - Invite Checksum256
    /// * `secret` - Secret in invite.
    #[action]
    fn inv_g_member(
        guild: AccountNumber,
        invite_id: u32,
    ...
    fn del_g_inv(invite_id: u32) {
        GuildInvite::get_assert(invite_id).cancel();
    }

    /// Set guild application.
    ///
    ///
    /// # Arguments
    /// * `guild_account` - Guild being applied for.
    /// * `extra_info` - Application description
    #[action]
