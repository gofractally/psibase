...
export const isD3ifiedLink = (
    link: AttestationGraphLink | AttestationGraphSecondaryLink,
): link is AttestationGraphLink => {
    return (
        (link as AttestationGraphLink).source.id !== undefined &&
        (link as AttestationGraphLink).target.id !== undefined
    );
};
