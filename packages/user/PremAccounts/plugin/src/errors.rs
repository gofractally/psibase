...
plugin_error! {
    ...
    ConfigMarketCallerDenied => "name market admin actions may only be called from the config app",
    CreateMarketLengthInvalid => "market name length is outside the allowed range for account names",
    ...
    PremiumNameMarketDisabled => "premium names of this length cannot be purchased at the moment",
}
