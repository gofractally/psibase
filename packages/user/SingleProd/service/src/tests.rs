...
mod tests {
    ...
    fn assert_private_network(chain: &psibase::Chain) -> Result<(), psibase::Error> {
    ...
    #[psibase::test_case]
    fn boots_with_single_prod() -> Result<(), psibase::Error> {
        let chain = Chain::new();
        chain.boot_with(
            &Chain::test_registry(),
            &["TestDefault".to_string(), "SingleProd".to_string()],
        )?;
        assert_private_network(&chain)
    }
}
