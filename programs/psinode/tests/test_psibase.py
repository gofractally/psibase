    ...
    def service(self, name, wasm=None, flags=[], server=None, data=[]):
        if wasm is not None:
            ...
            if len(flags) != 0 or server is not None:
                self.add_file(
                    "service/" + name + ".json",
                    json.dumps({"flags": flags, "server": server}),
                )
...
class Foo:
    def __init__(self):
        self.foo10 = (
            TestPackage("foo", "1.0.0", description="The original foo")
            .depends("Sites")
            .service("foo", data={"file1.txt": "original", "file2.txt": "deleted"})
        )
        self.foo11 = (
            TestPackage("foo", "1.1.0", description="Minor version update")
            .depends("Sites")
            .service("foo", data={"file1.txt": "updated", "file3.txt": "added"})
        )
        self.foo20 = (
            TestPackage("foo", "2.0.0", description="Major version update")
            .depends("Sites")
            .service("foo", data={"file1.txt": "version 2"})
        )

        # These just need to be valid and distinct
        self.original_wasm = bytes.fromhex(
            "0061736d01000000010a0260017e0060027e7e000303020001071d0305737461727400000663616c6c65640001086f726967696e616c00000a070202000b02000b"
        )
        self.updated_wasm = bytes.fromhex(
            "0061736d01000000010a0260017e0060027e7e000303020001071c0305737461727400000663616c6c65640001077570646174656400000a070202000b02000b"
        )
        self.deleted_wasm = bytes.fromhex(
            "0061736d01000000010a0260017e0060027e7e000303020001071c0305737461727400000663616c6c656400010764656c6574656400000a070202000b02000b"
        )
        self.added_wasm = bytes.fromhex(
            "0061736d01000000010a0260017e0060027e7e000303020001071a0305737461727400000663616c6c6564000105616464656400000a070202000b02000b"
        )

        self.foo10.service("bar1", wasm=self.original_wasm, flags=["isPrivileged"])
        self.foo10.service(
            "bar2", wasm=self.deleted_wasm, flags=["isPrivileged"], server="bar1"
        )
...
class TestPsibase(unittest.TestCase):
    ...
    def test_install(self, cluster):
        ...
        a.graphql(
            "tokens",
            """query { userBalances(user: "alice") { edges { node { symbol tokenId balance precision } } } }""",
        )
    ...
    def test_install_upgrade(self, cluster):
        ...
        with tempfile.TemporaryDirectory() as dir:
            ...
            a.run_psibase(
                ["install"] + a.node_args() + ["foo", "--package-source", dir]
            )
            a.wait(new_block())
    ...
    def test_info(self, cluster):
        ...
        with tempfile.TemporaryDirectory() as dir:
            ...
            def get_info(package):
                return a.run_psibase(
                    ["info"] + a.node_args() + [package, "--package-source", dir],
                    stdout=subprocess.PIPE,
                    encoding="utf-8",
                ).stdout
            ...
            a.run_psibase(
                ["install"] + a.node_args() + ["foo-1.0", "--package-source", dir]
            )
            a.wait(new_block())
            ...
            a.run_psibase(
                ["install"] + a.node_args() + ["foo-2.0", "--package-source", dir]
            )
            a.wait(new_block())
    ...
    def test_list(self, cluster):
        ...
        with tempfile.TemporaryDirectory() as dir:
            ...
            def get_list(*options):
                return a.run_psibase(
                    ["list", "--package-source", dir] + list(options) + a.node_args(),
                    stdout=subprocess.PIPE,
                    encoding="utf-8",
                ).stdout
    ...
    def test_search(self, cluster):
        ...
        with tempfile.TemporaryDirectory() as dir:
            ...
            def search(*options):
                return a.run_psibase(
                    ["search", "--package-source", dir] + list(options) + a.node_args(),
                    stdout=subprocess.PIPE,
                    text=True,
                ).stdout
    ...
    def test_configure_sources(self, cluster):
        ...
        foo10 = (
            TestPackage("foo", "1.0.0")
            .depends("Sites")
            .service("foo", data={"file1.txt": "data"})
        )
        with tempfile.TemporaryDirectory() as dir:
            path = os.path.join(dir, foo10.write(dir)["file"])
            a.run_psibase(["publish", "-S", "root"] + a.node_args() + [path])
            a.push_action(
                "root", "packages", "setSources", {"sources": [{"account": "root"}]}
            )
            a.wait(new_block())
