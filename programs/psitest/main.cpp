...
using __wasi_oflags_t = uint32_t; // uint16_t;
...
struct callbacks
{
   ...
   int32_t testerWasi_args_sizes_get(wasm_ptr<__wasi_size_t> argc, wasm_ptr<__wasi_size_t> argv_buf_size)
   {
      uint32_t size = 0;
   ...
   uint32_t testerWasi_fd_fdstat_get(__wasi_fd_t fd, wasm_ptr<__wasi_fdstat_t> stat)
   {
      if (fd == STDIN_FILENO)
      {
   ...
   uint32_t testerWasi_fd_fdstat_set_flags(__wasi_fd_t fd, uint32_t flags)
   {
      // TODO: Implement
      return 0;
   }

   uint32_t testerWasi_fd_readdir(__wasi_fd_t fd,
                                wasm_ptr<uint8_t> buf,
                                __wasi_size_t buf_len,
                                __wasi_dircookie_t cookie,
                                wasm_ptr<__wasi_size_t> retptr0)
   {
      throw std::runtime_error("__wasi_fd_readdir not implemented");
   }

   uint32_t testerWasi_fd_seek(__wasi_fd_t fd,
                             __wasi_filedelta_t offset,
                             uint32_t whence, // uint8_t
                             wasm_ptr<__wasi_filesize_t> newoffset)
   {
      throw std::runtime_error("__wasi_fd_seek not implemented");
   }

   uint32_t testerWasi_fd_filestat_get(__wasi_fd_t fd, wasm_ptr<__wasi_filestat_t> stat_ptr)
   {
      wasi_stat result = {};
   ...
   uint32_t testerWasi_fd_prestat_dir_name(__wasi_fd_t fd, wasm_ptr<uint8_t> path, __wasi_size_t path_len)
   {
      if (fd != polyfill_root_dir_fd)
         return wasi_errno_badf;
      if (path_len > 0)
   ...
   uint32_t testerWasi_fd_prestat_get(__wasi_fd_t fd, wasm_ptr<__wasi_prestat_t> buf)
   {
      if (fd != polyfill_root_dir_fd)
         return wasi_errno_badf;
   ...
   uint32_t testerWasi_path_filestat_get(__wasi_fd_t          fd,
                                      __wasi_lookupflags_t flags,
                                      span<const char> path,
                                      wasm_ptr<__wasi_filestat_t> stat_ptr)
   {
      if (fd != polyfill_root_dir_fd)
         return wasi_errno_badf;
      struct stat stat;
   ...
   uint32_t testerWasi_path_open(__wasi_fd_t fd,
                               __wasi_lookupflags_t dirflags,
                               span<const char>  path,
                               __wasi_oflags_t   oflags,
                               __wasi_rights_t   fs_rights_base,
                               __wasi_rights_t   fs_rights_inherting,
                               uint32_t  fdflags, // __wasi_fdflags_t u16
   ...
   uint32_t testerWasi_fd_close(__wasi_fd_t fd)
   {
   ...
   uint32_t testerWasi_fd_write(__wasi_fd_t fd, 
                              wasm_ptr<const __wasi_iovec_t> iovs, 
                              __wasi_size_t iovs_len, 
                              wasm_ptr<__wasi_size_t> nwritten)
   {
      if (nwritten)
         *nwritten = 0;
      ...
      for (size_t i = 0; i < iovs_len; ++i)
      {
         ...
         char* memory   = state.backend.get_context().linear_memory();
         void* iovs_buf = memory + iovs[i].buf;

         if (iovs[i].buf_len && fwrite(iovs_buf, iovs[i].buf_len, 1, file->f) != 1) {
            return wasi_errno_badf;
         }
   ...
   uint32_t testerWasi_fd_read(__wasi_fd_t fd, wasm_ptr<const __wasi_iovec_t> iovs, __wasi_size_t iovs_len, wasm_ptr<__wasi_size_t> nread)
   {
      if (nread)
         *nread = 0;
      ...
      {         
         ...
         size_t bytes_read = fread(iovs_buf, 1, iovs[i].buf_len, file->f);

         if (nread)
            *nread += bytes_read;
      
         if (bytes_read < iovs[i].buf_len)
            return 0;
      }
