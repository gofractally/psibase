...
struct TxArgs {
    ...
    /// Sender to use (required). Installs are owned by this account. Files are uploaded to this account's subdomain.
    // TODO: need different defaults, different required flags, and different help texts...
...
async fn create(args: &CreateArgs) -> Result<(), anyhow::Error> {
    ...
    actions.push(new_account_action(
        args.tx_args.sender.into(),
        args.account.into(),
    ));
...
async fn upload(args: &UploadArgs) -> Result<(), anyhow::Error> {
    let (client, _proxy) = build_client(&args.node_args.proxy).await?;
    if args.tx_args.sender == AccountNumber::from("").into() {
        return Err(anyhow!("sender is required"));
    }
...
async fn install(args: &InstallArgs) -> Result<(), anyhow::Error> {
    ...
    let build_transaction = |mut actions: Vec<Action>| -> Result<SignedTransaction, anyhow::Error> {
        if actions.first().unwrap().sender != args.tx_args.sender.into() {
            actions.insert(
                0,
                Action {
                    sender: args.tx_args.sender.into(),
                    service: account!("nop"),
                    method: method!("nop"),
