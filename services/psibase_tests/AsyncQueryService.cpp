...
PSIO_REFLECT(AsyncResponseRow, socket, reply)
...
struct AsyncQueryService : psibase::Service<AsyncQueryService>
{
   ...
   void onBlock();
   auto serveSys(const HttpRequest& req, std::optional<std::int32_t> socket) -> std::optional<HttpReply>;
};
...
std::optional<HttpReply> AsyncQueryService::serveSys(const HttpRequest& request, std::optional<std::int32_t> socket)
{
