...
namespace SystemService
{
   ...
   void AuthBasic::checkAuthSys(uint32_t                    flags,
   ...
   void AuthBasic::canAuthUserSys(psibase::AccountNumber user)
   {
      // Anyone with a public key in the AuthTable may use AuthBasic
      auto row = db.open<AuthTable>().getIndex<0>().get(user);
      check(row.has_value(), "sender does not have a public key");
   }

   void AuthBasic::setKey(SubjectPublicKeyInfo key)
   {
...
PSIBASE_DISPATCH(SystemService::AuthBasic)
