#include <services/system/RAuthBasic.hpp>
...
namespace SystemService
{
   ...
   struct AuthQuery
   {
      ...
      {
         auto idx = AuthBasic::Tables{AuthBasic::service}
                        .open<AuthBasic::AuthTable>()
                        .getIndex<1>()
                        .subindex(key);
      ...
      {
         AuthBasic::Tables db{AuthBasic::service};
         auto              account = db.open<AuthBasic::AuthTable>().get(name);
   ...
   std::optional<HttpReply> RAuthBasic::serveSys(HttpRequest request)
   {
      ...
      if (auto result = serveSimpleUI<AuthBasic, true>(request))
         return result;
   ...
   void RAuthBasic::storeSys(std::string path, std::string contentType, std::vector<char> content)
   {
...
PSIBASE_DISPATCH(SystemService::RAuthBasic)
