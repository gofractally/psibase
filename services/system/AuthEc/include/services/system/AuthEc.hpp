...
namespace SystemService
{
   ...
   {  // Using a namespace here disambiguates this class from the one in AuthBasic so docs don't get confused
   ...
   /// The `auth-ec` service is an auth service that can be used to authenticate actions for accounts
   ///
   ...
   class AuthEc : public psibase::Service<AuthEc>
   {
     public:
      static constexpr auto service = psibase::AccountNumber("auth-ec");
      ...
      ///
      /// This action allows any user who has already set a public key using `AuthEc::setKey`.
