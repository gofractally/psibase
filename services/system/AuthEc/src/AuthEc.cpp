...
{
   void AuthEc::checkAuthSys(uint32_t                    flags,
   ...
   void AuthEc::canAuthUserSys(psibase::AccountNumber user)
   {
      // Anyone with a public key in the AuthTable may use AuthEc
      auto row = db.open<AuthTable>().getIndex<0>().get(user);
      check(row.has_value(), "sender does not have a public key");
   }

   void AuthEc::setKey(psibase::PublicKey key)
   {
...
PSIBASE_DISPATCH(SystemService::AuthEc)
