...
pub mod service {
    ...
    fn create_group(
        ...
        check(
            ...
            "Each user must be given the secret to generate the symmetric key",
        );
        check(
            members.len() > 1,
    ...
    /// # Arguments
    /// * `id` - The group id
    /// * `secrets` - The new secret, encrypted for each user in the group using their public encryption key
    ///               This secret is used with a KDF to generate the new symmetric key for the group.
    ...
    fn rotate_key(id: Checksum256, secrets: Vec<Vec<u8>>, key_digest: Checksum256) {
        ...
        group.rotate_key(secrets, key_digest);
