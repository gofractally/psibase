...
/// This interface extracts some common functionality needed by the developer of a plugin prompt UI.
export const prompt = {

  /// Get the contextId specified by your plugin when it triggered the prompt.
  /// A contextId is often used to look up (within your plugin) additional inforamtion about
  ...
  /// Call this when the user has finished interacting with your prompt. It will redirect the 
  /// user back to the app from which they came.
  finished(): void {
      window.parent.postMessage('finished', supervisorUrl);
  }
};

/// This function is used by root app UIs to automatically facilitate redirecting the 
