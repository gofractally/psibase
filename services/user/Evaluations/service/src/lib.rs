...
pub mod service {
    ...
    ///
    /// Sorts participating accounts by AccountNumber then shuffles by evaluation ID as deterministic seed then chunks
    /// them into groups from the `allowed_group_sizes` var.
    /// 1:1 mapping between the shuffled vector and the group sizes generated by the gmp algorithm in the subgroups service.
    ///
    /// This means anyone is free to calculate expected groupings for a given set of registered participants ahead of time, and anyone registering can have a deterministic effect on the final grouping by creating
    /// and registering with a carefully chosen account number.
