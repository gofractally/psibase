...
const addBackAttestationsToGraph = (
    nodesInGraph: AttestationGraphNode[],
    linksInGraph: AttestationGraphLink[],
    graphData: AttestationGraph,
) => {
    const ns = nodesInGraph;
    const ls = linksInGraph;
    // 1. get all attestations that target nodes in the graph
    const backAttestations = graphData.attestations.filter((L) =>
        ns.find((n) => n.id === L.target.id),
    );
    // 2. filter out attestations that are *forward* attestations, i.e., already in the ls list
    return backAttestations.filter(
        (bl) =>
            !ls.find(
                (L) =>
                    L.source.id === bl.source.id &&
                    L.target.id === bl.target.id,
            ),
    );
};

export const getCouncilLinks = (
    ...
    councilLinks.sort(
        (a, b) =>
            (a.source.id - b.source.id) * 100 + (a.target.id - b.target.id),
    );
    let l_prev: AttestationGraphLink | undefined = undefined;
    councilLinks = councilLinks.filter((l) => {
        if (l.target !== l_prev?.target || l.source !== l_prev?.source) {
            l_prev = l;
            return true;
        } else {
            return false;
...
function recurseForCouncilLinks(
    graph: AttestationGraph,
    node: AttestationGraphNode,
    depth: number,
    targetDepth: number,
    pathLinks: AttestationGraphLink[],
) {
...
export const convertCyclicToAcyclicGraph = (
    orig_graph: AttestationGraph,
    { centeredNodeIdx, dagDepth }: UiOptions,
) => {
    ...
    while (queue[depth].length > 0 && iterations < 30) {
        ...
        const nextNodes: AttestationGraphNode[] = nextAttestations.reduce(
            (acc: AttestationGraphNode[], l) => {
    ...
    return {
        nodes: graph.nodes,
        attestations: ls,
        backAttestations: backAttestations,
    };
};
